...
class MultiplyImages(shell.Task["MultiplyImages.Outputs"]):
    ...
    >>> from fileformats.generic import File
    >>> from pathlib._local import Path
    >>> from pydra.tasks.ants.autonipype.interfaces.ants.utils.multiply_images import MultiplyImages
    ...
    dimension: ty.Any = shell.arg(
        help="image dimension (2 or 3)", argstr="{dimension}", position=0
    )
    first_input: File = shell.arg(help="image 1", argstr="{first_input}", position=1)
    second_input: ty.Any = shell.arg(
        help="image 2 or multiplication weight", argstr="{second_input}", position=2
    )
