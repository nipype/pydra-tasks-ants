...
class AffineInitializer(shell.Task["AffineInitializer.Outputs"]):
    ...
    >>> from fileformats.medimage import Nifti1
    >>> from pathlib._local import Path
    >>> from pydra.tasks.ants.autonipype.interfaces.ants.utils.affine_initializer import AffineInitializer
    ...
    dimension: ty.Any = shell.arg(
        help="dimension", argstr="{dimension}", position=0, default=3
    )
    fixed_image: Nifti1 = shell.arg(
        help="reference image", argstr="{fixed_image}", position=1
    )
    moving_image: File = shell.arg(
        help="moving image", argstr="{moving_image}", position=2
    )
