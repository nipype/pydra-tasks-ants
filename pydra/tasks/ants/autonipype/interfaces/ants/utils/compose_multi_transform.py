...
class ComposeMultiTransform(shell.Task["ComposeMultiTransform.Outputs"]):
    ...
    >>> from fileformats.generic import File
    >>> from pathlib._local import Path
    >>> from pydra.tasks.ants.autonipype.interfaces.ants.utils.compose_multi_transform import ComposeMultiTransform
    ...
    transforms: list[File] = shell.arg(
        help="transforms to average", argstr="{transforms}", position=3
    )
