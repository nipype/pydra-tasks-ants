...
class ConvertScalarImageToRGB(shell.Task["ConvertScalarImageToRGB.Outputs"]):
    ...
    >>> from fileformats.generic import File
    >>> from pydra.tasks.ants.autonipype.interfaces.ants.visualization.convert_scalar_image_to_rgb import ConvertScalarImageToRGB
    ...
    dimension: ty.Any | None = shell.arg(
        help="image dimension (2 or 3)", argstr="{dimension}", position=0, default=3
    )
    input_image: File = shell.arg(
        help="Main input is a 3-D grayscale image.", argstr="{input_image}", position=1
    )
    ...
    mask_image: ty.Any = shell.arg(
        help="mask image", argstr="{mask_image}", position=3, default="none"
    )
    colormap: ty.Any = shell.arg(
        help="Select a colormap", argstr="{colormap}", position=4
    )
    ...
    minimum_input: int = shell.arg(
        help="minimum input", argstr="{minimum_input}", position=6
    )
    maximum_input: int = shell.arg(
        help="maximum input", argstr="{maximum_input}", position=7
    )
    minimum_RGB_output: int = shell.arg(
        help="", argstr="{minimum_RGB_output}", position=8, default=0
    )
    maximum_RGB_output: int = shell.arg(
        help="", argstr="{maximum_RGB_output}", position=9, default=255
    )
